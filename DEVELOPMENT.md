...
- **Trigger name**: `trg_indices_{descriptive_name}`
- **Function name**: `trg_indices_{descriptive_name}_fn`

Where `indices` is your plugin's moduleId (pluginId with hyphens replaced by underscores).
...
CREATE OR REPLACE FUNCTION "trg_indices_audit_log_fn"()
RETURNS TRIGGER AS $$
BEGIN
...
CREATE TRIGGER "trg_indices_audit_log"
AFTER INSERT OR UPDATE ON "module_indices_items"
FOR EACH ROW
EXECUTE FUNCTION "trg_indices_audit_log_fn"();
```

> **Important**: The system automatically **DISABLE**s all `trg_indices_*` triggers when the plugin
