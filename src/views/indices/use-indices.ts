/**
 * Índices — datos y estado (generado por el Builder de Vistas).
 *
...
export function useIndicesView() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const byBlock = customHandlers.loadDataFor?.['tbl_valores'];
      const data = byBlock
        ? await byBlock(ctx)
        : customHandlers.loadData
          ? await customHandlers.loadData(ctx)
          : await actions.execute<any[]>('indices.values.list');
  ...
  useEffect(() => {
    const offs = [
      'indices.values.create',
      'indices.values.update',
      'indices.values.delete',
      'indices.values.restore',
    ].map((id) =>
      events.on(id, () => {
        void load();
      })
  ...
  const COLUMNS: {
    ...
    { key: 'value_date', label: 'Fecha', format: 'date' },
    {
      key: 'index_code',
      label: 'Índice',
      display: 'pill',
  ...
  const confirmDelete = useCallback(async () => {
    ...
    setDeleting(true);
    try {
      await actions.execute('indices.values.delete', { id: pendingDelete.id });
